...
clean_data <- function(df, keep_lat_above=49) {
  #' Clean data frame
  #' 
  #' filters data to keep points with latitude above @param keep_lat_above
  #' 
  ...
  return(df[which (df$latitude > keep_lat_above), ])
}
```
...
```{r}
wildfires_df <- geojson_read("https://github.com/beeduino/wildfires-experiments/raw/main/data/wildfires_viirs_data_russia-asia-24h_2021-09-30.geojson", what="sp")
...
```{r echo=FALSE}
leaflet(wildfires_df) %>%
  addTiles() %>%
  addCircleMarkers(radius = 2, color = "orange", stroke=FALSE, fillOpacity=0.3)
```
and filtered
```{r echo=FALSE}
leaflet(clean_data(wildfires_df)) %>%
  addTiles() %>%
  addCircleMarkers(radius = 2, color = "orange", stroke=FALSE, fillOpacity=0.3)
```
...
```{r echo=FALSE}
wildfires_firms_df = read.csv("https://firms.modaps.eosdis.nasa.gov/data/active_fire/suomi-npp-viirs-c2/csv/SUOMI_VIIRS_C2_Russia_Asia_24h.csv")
```

Then plot it

```{r echo=FALSE}
leaflet(clean_data(wildfires_firms_df)) %>%
  addTiles() %>%
  addCircleMarkers(radius = 2, color = "red", stroke=FALSE, fillOpacity=0.2)
